  ...
  83: [
    {
      kind: 'p',
      text:
        'And five bolder calls, on a longer clock — direction is the claim, timing is the error bar, and the dates are deliberately loose because the arithmetic says they must be. The largest meter today bills about $4.5 billion a year; $100 billion is a twenty-fold, which takes roughly nine years at wave-case growth and twelve at 30% — hence the mid-2030s, not sooner. The trillion-dollar call is measured plainly — the summed annual revenue of the 25 public backend names, roughly $60 billion today, re-summed quarterly on this site — and a seventeen-fold at 25–30% compounding lands in the late 2030s. The out-earning call rests on a mechanism rather than a date: the labs cut their unit price 30–40% a year by design, while the meters’ events per token rise as workloads reach production — engines compete on price, toll roads compound on volume, and every prior mobilization ended with the toll collectors out-earning the engine builders. Today the labs bill roughly three dollars for every dollar the cohort meters, and lab revenue is still growing faster; we track the ratio quarterly precisely because the crossover, not the calendar, is the claim.',
    },
  ],
